...
def demo_test():
    ...
    if 'user' not in st.session_state:
        st.session_state.user = {'id': 'demo_user', 'name': '演示用户'}
    ...
    with col1:
        if st.button("添加草药A"):
            Backpack.add_item(user_id, "herb_a", 10)
            st.success("已添加10份草药A")
    
    with col2:
        if st.button("添加灵泉水"):
            Backpack.add_item(user_id, "water_b", 5)
            st.success("已添加5份灵泉水")
    
    with col3:
        if st.button("添加铁矿石"):
            Backpack.add_item(user_id, "iron_ore", 10)
            st.success("已添加10份铁矿石")
    
    with col4:
        if st.button("添加煤炭"):
            Backpack.add_item(user_id, "coal", 5)
            st.success("已添加5份煤炭")
    ...
    if inventory:
        for inv_item in inventory:
            ...
            item_detail = None
            # 在配方中查找
            for recipe in AlchemySystem.get_recipes():
                if recipe.result_item.id == inv_item['item_id']:
                    item_detail = recipe.result_item
                    break
                elif recipe.material_1.id == inv_item['item_id']:
                    item_detail = recipe.material_1
                    break
                elif recipe.material_2 and recipe.material_2.id == inv_item['item_id']:
                    item_detail = recipe.material_2
                    break
            
            # 在图纸中查找
            if not item_detail:
                for bp in ForgeSystem.get_blueprints():
                    if bp.result_item.id == inv_item['item_id']:
                        item_detail = bp.result_item
                        break
                    elif bp.material_1.id == inv_item['item_id']:
                        item_detail = bp.material_1
                        break
                    elif bp.material_2 and bp.material_2.id == inv_item['item_id']:
                        item_detail = bp.material_2
                        break
